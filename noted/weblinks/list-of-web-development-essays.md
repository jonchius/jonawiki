# List of web development essays

* [14 software architecture design patterns to know](https://www.redhat.com/en/blog/14-software-architecture-patterns)
* [why (full text) search is hard](https://transactional.blog/blog/2023-why-full-text-search-is-hard) (in some languages)&#x20;
* [technology is probably changing us for the worse (or so we always think)](https://www.technologyreview.com/2024/05/15/1092350/technology-is-probably-changing-us-for-the-worse-or-so-we-always-think/)
* [good code is rarely read](https://www.alexmolas.com/2024/06/06/good-code.html)
* [goodbye clean code](https://overreacted.io/goodbye-clean-code/)
* [the importance of graceful degradation](https://www.smashingmagazine.com/2024/12/importance-graceful-degradation-accessible-interface-design/)
* [will the syrian flag emoji be changed? if so, when?](https://blog.emojipedia.org/will-the-syrian-flag-emoji-be-changed-when/)
